...
window.onload = () => {
    queue.on("complete",preloadImage,this);
    queue.loadManifest([
...
function preloadImage(){
    ...
    //プレイヤー1の枠
    let player1frame = new createjs.Bitmap();
    player1frame.image = queue.getResult("evenPlayerFrame");
    player1frame.regX = player1frame.image.width / 2;
    player1frame.regY = player1frame.image.height;
    player1frame.x = stage.canvas.width / 2;
    player1frame.y = stage.canvas.height;
    stage.addChild(player1frame);
    //プレイヤー1の設置アクション
    let player1buildArea = new createjs.Bitmap();
    player1buildArea.image = queue.getResult("oddPlayerRBArea");
    player1buildArea.regX = player1buildArea.image.width / 2;
    player1buildArea.regY = player1buildArea.image.height;
    player1buildArea.x = stage.canvas.width / 2;
    player1buildArea.y = stage.canvas.height - player1frame.image.height - 4;
    stage.addChild(player1buildArea);
    //プレイヤー1のリソース
    let player1resourceArea = new createjs.Bitmap();
    player1resourceArea.image = queue.getResult("oddPlayerRBArea");
    player1resourceArea.regX = player1resourceArea.image.width / 2;
    player1resourceArea.regY = player1resourceArea.image.height;
    player1resourceArea.x = stage.canvas.width / 2;
    player1resourceArea.y = player1buildArea.y - player1buildArea.image.height - 4;
    stage.addChild(player1resourceArea);

    //プレイヤー2の枠
    let player2frame = new createjs.Bitmap();
    player2frame.image = queue.getResult("oddPlayerFrame");
    player2frame.regY = player2frame.image.height / 2;
    player2frame.y = stage.canvas.height / 2;
    stage.addChild(player2frame);

    //プレイヤー3の枠
    let player3frame = new createjs.Bitmap();
    player3frame.image = queue.getResult("evenPlayerFrame");
    player3frame.rotation = 180;
    player3frame.regX = player3frame.image.width / 2;
    player3frame.regY = 0;
    player3frame.x = stage.canvas.width / 2;
    player3frame.y = player3frame.image.height;
    stage.addChild(player3frame);

    //プレイヤー4の枠
    let player4frame = new createjs.Bitmap();
    player4frame.image = queue.getResult("oddPlayerFrame");
    player4frame.rotation = 180;
    player4frame.regX = player4frame.image.width;
    player4frame.regY = player4frame.image.height / 2;
    player4frame.x = stage.canvas.width - player4frame.image.width;
    player4frame.y = stage.canvas.height / 2;
    stage.addChild(player4frame);

    //設定枠
    let topWindowsL = new createjs.Bitmap();
    topWindowsL.image = queue.getResult("topWindows");
    stage.addChild(topWindowsL);
    //設定ボタン
    let setting = new createjs.Bitmap();
    setting.image = queue.getResult("setting");
    ...
    //イベント枠
    let topWindowsR = new createjs.Bitmap();
    topWindowsR.image = queue.getResult("topWindows");
    topWindowsR.scaleX = -1;
    topWindowsR.x = stage.canvas.width;
